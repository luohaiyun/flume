	...
	public static String getFileId(String filepath) {
		...
		if (Kernel32.INSTANCE.GetLastError() == 0) {
			...
			if (Kernel32.INSTANCE.GetLastError() == 0) {
				...
				logger.error("close fail filepath: {}, error:{}", filepath, Kernel32.INSTANCE.GetLastError());
				throw new RuntimeException("close fail filepath: " + filepath + " error:" + Kernel32.INSTANCE.GetLastError());
			}
		} else {
			if (hFile != null) {
				Kernel32.INSTANCE.CloseHandle(hFile);
			}
			logger.error("open fail filepath:{}, error:{}", filepath, Kernel32.INSTANCE.GetLastError());
			throw new RuntimeException("open fail filepath: " + filepath + " error:" + Kernel32.INSTANCE.GetLastError());
		}
